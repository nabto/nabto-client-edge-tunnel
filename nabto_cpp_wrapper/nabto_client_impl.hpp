...
class FutureBufferImpl : public FutureBuffer,
                         public std::enable_shared_from_this<FutureBufferImpl> {
public:
  FutureBufferImpl(NabtoClient *context,
                   std::shared_ptr<std::vector<uint8_t>> data,
                   std::shared_ptr<size_t> transferred)
      : future_(nabto_client_future_new(context)), data_(data),
        transferred_(transferred) {}
  FutureBufferImpl(NabtoClientFuture *future,
                   std::shared_ptr<std::vector<uint8_t>> data,
                   std::shared_ptr<size_t> transferred)
  ...
  ~FutureBufferImpl() {
    if (!ended_) {
      ...
      c->callback(
          std::make_shared<CallbackFunction>([](Status) { /* do nothing */ }));
    } else {
  ...
  static void doCallback(NabtoClientFuture *future, NabtoClientError ec,
                         void *data) {
  ...
  void callback(std::shared_ptr<FutureCallback> cb) {
    ...
    nabto_client_future_set_callback(future_, &doCallback, this);
  }
  ...
  NabtoClientFuture *getFuture() { return future_; }
...
public:
  MdnsResultImpl(NabtoClientMdnsResult *result) : result_(result) {}
  ~MdnsResultImpl() { nabto_client_mdns_result_free(result_); }
  ...
  virtual std::string getServiceInstanceName() {
    return std::string(
        nabto_client_mdns_result_get_service_instance_name(result_));
  }
  ...
  virtual MdnsResult::Action getAction() {
    ...
    switch (action) {
    case NABTO_CLIENT_MDNS_ACTION_ADD:
      return MdnsResult::Action::ADD;
    case NABTO_CLIENT_MDNS_ACTION_UPDATE:
      return MdnsResult::Action::UPDATE;
    case NABTO_CLIENT_MDNS_ACTION_REMOVE:
...
class FutureMdnsResultImpl
    : public FutureMdnsResult,
      public std::enable_shared_from_this<FutureMdnsResultImpl> {
public:
  FutureMdnsResultImpl(NabtoClient *context)
      : future_(nabto_client_future_new(context)) {}
  FutureMdnsResultImpl(NabtoClientFuture *future) : future_(future) {}
...
class FutureVoidImpl : public FutureVoid,
                       public std::enable_shared_from_this<FutureVoidImpl> {
public:
  ...
  FutureVoidImpl(NabtoClientFuture *future) : future_(future) {}
  ~FutureVoidImpl() {
    if (!ended_) {
...
public:
  ...
  ~MdnsResolverImpl() { nabto_client_listener_free(resolver_); }
  virtual std::shared_ptr<FutureMdnsResult> getResult() {
    auto future = std::make_shared<FutureMdnsResultImpl>(context_);
    nabto_client_listener_new_mdns_result(resolver_, future->getFuture(),
                                          &future->result_);
    return future;
  }
  virtual void stop() { nabto_client_listener_stop(resolver_); }
...
public:
  CoapImpl(NabtoClient *context, NabtoClientCoap *coap) : context_(context) {
    request_ = coap;
  }
  ~CoapImpl() { nabto_client_coap_free(request_); };

  static std::shared_ptr<CoapImpl> create(NabtoClient *context,
                                          NabtoClientConnection *connection,
                                          const std::string &method,
                                          const std::string &path) {
    auto request_ =
        nabto_client_coap_new(connection, method.c_str(), path.c_str());
    if (!request_) {
      return nullptr;
    }
  ...
  void setRequestPayload(int contentFormat,
                         const std::vector<uint8_t> &payload) {
    NabtoClientError ec;
    ec = nabto_client_coap_set_request_payload(request_, contentFormat,
  ...
  int getResponseContentFormat() {
    ...
    ec =
        nabto_client_coap_get_response_content_format(request_, &contentFormat);
  ...
  std::vector<uint8_t> getResponsePayload() {
    ...
    NabtoClientError ec = nabto_client_coap_get_response_payload(
        request_, &payload, &payloadLength);
...
public:
  ...
  ~StreamImpl() { nabto_client_stream_free(stream_); }
  ...
  std::shared_ptr<FutureBuffer> readAll(size_t n) {
    ...
    auto future =
        std::make_shared<FutureBufferImpl>(context_, data, transferred);
    nabto_client_stream_read_all(stream_, future->getFuture(), data->data(),
                                 data->size(), transferred.get());
    return future;
  }
  std::shared_ptr<FutureBuffer> readSome(size_t max) {
    ...
    auto future =
        std::make_shared<FutureBufferImpl>(context_, data, transferred);
    nabto_client_stream_read_some(stream_, future->getFuture(), data->data(),
                                  data->size(), transferred.get());
    return future;
  }
  std::shared_ptr<FutureVoid> write(const std::vector<uint8_t> &buffer) {
    auto data =
        std::make_shared<std::vector<uint8_t>>(buffer.begin(), buffer.end());
    auto future = std::make_shared<FutureVoidImpl>(context_, data);
    nabto_client_stream_write(stream_, future->getFuture(), data->data(),
                              data->size());
    return future;
  }
  ...
  void abort() { nabto_client_stream_abort(stream_); }
...
public:
  ...
  virtual ~TcpTunnelImpl() { nabto_client_tcp_tunnel_free(tcpTunnel_); };
  virtual std::shared_ptr<FutureVoid> open(const std::string &service,
                                           uint16_t localPort) {
    auto future = std::make_shared<FutureVoidImpl>(context_);
    nabto_client_tcp_tunnel_open(tcpTunnel_, future->getFuture(),
                                 service.c_str(), localPort);
    return future;
  }
  ...
  virtual uint16_t getLocalPort() {
    uint16_t localPort;
    NabtoClientError ec =
        nabto_client_tcp_tunnel_get_local_port(tcpTunnel_, &localPort);
    if (ec) {
      throw NabtoException(ec);
    }
    return localPort;
...
class ConnectionEventsListenerImpl
    : public std::enable_shared_from_this<ConnectionEventsListenerImpl> {
public:
  ConnectionEventsListenerImpl(NabtoClient *context,
                               NabtoClientConnection *connection,
                               std::shared_ptr<ConnectionImpl> connectionImpl);

  void init() {
    NabtoClientError ec =
        nabto_client_connection_events_init_listener(connection_, listener_);
    if (ec) {
      throw NabtoException(ec);
    }
    selfReference_ = shared_from_this();
  ...
  void listen() {
    ...
    nabto_client_future_set_callback(
        future_, ConnectionEventsListenerImpl::futureCallback, this);
  }

  void stopped() { selfReference_.reset(); }

  static void futureCallback(NabtoClientFuture *future, NabtoClientError ec,
                             void *data);

  void stop() { nabto_client_listener_stop(listener_); }
...
public:
  ConnectionImpl(NabtoClient *context) : context_(context) {
  ...
  void init() {
    connectionEventsListener_ = std::make_shared<ConnectionEventsListenerImpl>(
        context_, connection_, shared_from_this());
    connectionEventsListener_->init();
  }

  void setProductId(const std::string &productId) {
    NabtoClientError ec =
        nabto_client_connection_set_product_id(connection_, productId.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  void setDeviceId(const std::string &deviceId) {
    NabtoClientError ec =
        nabto_client_connection_set_device_id(connection_, deviceId.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  void setServerKey(const std::string &serverKey) {
    NabtoClientError ec =
        nabto_client_connection_set_server_key(connection_, serverKey.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  void setApplicationName(const std::string &applicationName) {
    NabtoClientError ec = nabto_client_connection_set_application_name(
        connection_, applicationName.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  void setApplicationVersion(const std::string &applicationVersion) {
    NabtoClientError ec = nabto_client_connection_set_application_version(
        connection_, applicationVersion.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  void setServerUrl(const std::string &serverUrl) {
    NabtoClientError ec =
        nabto_client_connection_set_server_url(connection_, serverUrl.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  void setServerJwtToken(const std::string &serverJwtToken) {
    NabtoClientError ec = nabto_client_connection_set_server_jwt_token(
        connection_, serverJwtToken.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  void setServerConnectToken(const std::string &serverConnectToken) {
    NabtoClientError ec = nabto_client_connection_set_server_connect_token(
        connection_, serverConnectToken.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  void setPrivateKey(const std::string &privateKey) {
    NabtoClientError ec = nabto_client_connection_set_private_key(
        connection_, privateKey.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  void setOptions(const std::string &options) {
    NabtoClientError ec =
        nabto_client_connection_set_options(connection_, options.c_str());
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  Connection::Type getType() {
    ...
    switch (type) {
    case NABTO_CLIENT_CONNECTION_TYPE_RELAY:
      return Connection::Type::RELAY;
    case NABTO_CLIENT_CONNECTION_TYPE_DIRECT:
  ...
  int getLocalChannelErrorCode() {
    return (int)nabto_client_connection_get_local_channel_error_code(
        connection_);
  }

  int getRemoteChannelErrorCode() {
    return (int)nabto_client_connection_get_remote_channel_error_code(
        connection_);
  }
  ...
  void enableDirectCandidates() {
    NabtoClientError ec =
        nabto_client_connection_enable_direct_candidates(connection_);
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  void addDirectCandidate(const std::string &hostname, uint16_t port) {
    NabtoClientError ec = nabto_client_connection_add_direct_candidate(
        connection_, hostname.c_str(), port);
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  void endOfDirectCandidates() {
    NabtoClientError ec =
        nabto_client_connection_end_of_direct_candidates(connection_);
    if (ec) {
      throw NabtoException(ec);
    }
  }
  ...
  std::shared_ptr<Coap> createCoap(const std::string &method,
                                   const std::string &path) {
  ...
  std::shared_ptr<FutureVoid>
  passwordAuthenticate(const std::string &username,
                       const std::string &password) {
    auto future = std::make_shared<FutureVoidImpl>(context_);
    nabto_client_connection_password_authenticate(
        connection_, username.c_str(), password.c_str(), future->getFuture());
    return future;
  }
  ...
  void
  removeEventsListener(std::shared_ptr<ConnectionEventsCallback> callback) {
    std::lock_guard<std::mutex> lock(mutex_);
...
public:
  ...
  static void cLogCallback(const NabtoClientLogMessage *message,
    ...
    if (logger) {
      LogMessageImpl msg =
          LogMessageImpl(message->message, message->severityString);
      logger->log(msg);
    }
  }
...
public:
  ContextImpl() { context_ = nabto_client_new(); }
  ~ContextImpl() {
...
std::string Context::version() { return std::string(nabto_client_version()); }
