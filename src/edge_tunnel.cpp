...
enum { COAP_CONTENT_FORMAT_APPLICATION_CBOR = 60 };

// TODO reconnect when connection is closed.

std::string generalHelp =
    R"(This client application is designed to be used with a tcp tunnel
...
void PrintGeneralHelp() { std::cout << generalHelp << std::endl; }

class MyLogger : public nabto::client::Logger {
public:
  void log(nabto::client::LogMessage message) {
    std::cout << time_in_HH_MM_SS_MMM() << " [" << message.getSeverity()
...
static void printMissingClientConfig(const std::string &filename) {
  std::cerr << "The example is missing the client configuration file ("
            << filename << ")." << std::endl
            << "The client configuration file is a json file which can be"
            << std::endl
            << "used to change the server URL used for remote connections."
            << std::endl
            << "In normal scenarios, the file should simply contain an"
            << std::endl
            << "empty json document:"
            << "{" << std::endl
            << "}" << std::endl;
...
void handleFingerprintMismatch(
  ...
  if (ec.ok()) {
    if (pairingInfo->getProductId() != device.getProductId()) {
      std::cerr << "The Product ID of the connected device ("
                << pairingInfo->getProductId()
                << ") does not match the Product ID for the bookmark "
                << device.getFriendlyName() << std::endl;
    } else if (pairingInfo->getDeviceId() != device.getDeviceId()) {
      std::cerr << "The Device ID of the connected device ("
                << pairingInfo->getDeviceId()
                << ") does not match the Device ID for the bookmark "
                << device.getFriendlyName() << std::endl;
    } else {
      std::cerr << "The public key of the device does not match the public key "
                   "in the pairing. Repair the device with the client."
                << std::endl;
    }
  } else {
    // should not happen
...
std::shared_ptr<nabto::client::Connection>
createConnection(std::shared_ptr<nabto::client::Context> context,
                 Configuration::DeviceInfo device) {
  ...
  try {
    ...
    if (e.status().getErrorCode() == nabto::client::Status::NO_CHANNELS) {
      auto localStatus =
          nabto::client::Status(connection->getLocalChannelErrorCode());
      auto remoteStatus =
          nabto::client::Status(connection->getRemoteChannelErrorCode());
      std::cerr << "Not Connected." << std::endl;
      std::cerr << " The Local status is: " << localStatus.getDescription()
                << std::endl;
      std::cerr << " The Remote status is: " << remoteStatus.getDescription()
                << std::endl;
    } else {
  ...
  try {
    ...
    std::cerr
        << "Missing device fingerprint in state, pair with the device again"
        << std::endl;
    return nullptr;
  }
  ...
  if (!user) {
    std::cerr << "The client is not paired with device, do the pairing again"
              << std::endl;
    return nullptr;
  }
  return connection;
...
static void get_service(std::shared_ptr<nabto::client::Connection> connection,
...
void get_service(std::shared_ptr<nabto::client::Connection> connection,
                 const std::string &service) {
...
void print_service(const nlohmann::json &service) {
  ...
  std::cout << "Service: " << constant_width_string(id)
            << " Type: " << constant_width_string(type) << " Host: " << host
            << "  Port: " << port << std::endl;
}

bool split_in_service_and_port(const std::string &in, std::string &service,
                               uint16_t &port) {
  ...
  if (colon != std::string::npos) {
    ...
    try {
      ...
      std::cerr << "the format for the service is not correct the string " << in
                << " cannot be parsed as service:port" << std::endl;
      return false;
    }
  } else {
    port = 0;
...
bool tcptunnel(std::shared_ptr<nabto::client::Connection> connection,
  ...
  for (auto serviceAndPort : services) {
    ...
    try {
      ...
      std::cout << "Failed to open a tunnel to " << serviceAndPort
                << " error: " << e.what() << std::endl;
      return false;
    }

    std::cout << "TCP Tunnel opened for the service " << service
              << " listening on the local port " << tunnel->getLocalPort()
              << std::endl;
...
int main(int argc, char **argv) {
  ...
  options.add_options("General")("h,help", "Show help")("version",
                                                        "Show version")(
      "H,home",
      "Override the directory in which configuration files are saved to.",
      cxxopts::value<std::string>())(
      "log-level", "Log level (none|error|info|trace)",
      cxxopts::value<std::string>()->default_value("error"));
  options.add_options("Bookmarks")("bookmarks", "List bookmarked devices")(
      "b,bookmark", "Select a bookmarked device to use with other commands.",
      cxxopts::value<uint32_t>()->default_value("0"))(
      "delete-bookmark", "Delete a pairing with a device");

  options.add_options("Pairing")("pair-local",
                                 "Pair the client with a tcp tunnel device "
                                 "running on the local network interactively")(
      "pair-string", "Pair with a tcp tunnel device using a pairing string",
      cxxopts::value<std::string>())(
      "pair-direct",
      "Pair with a tcp tunnel device directly using its ip or hostname",
      cxxopts::value<std::string>());

  options.add_options("IAM")("users", "List all users on selected device.")(
      "roles", "List roles available on device.")(
      "get-me", "Get the user associated with the current connection.")(
      "get-user", "Get a user.")("set-role", "Assign a role to a user.")(
      "delete-user", "Delete a user on device.")(
      "create-user", "Create a user new interactively in the device.")(
      "configure-open-pairing",
      "Configure pairing where users can register themselves in the device.")(
      "set-friendly-name", "Set the friendly name of the device.",
      cxxopts::value<std::string>())(
  ...
  options.add_options("TCP Tunnelling")(
      ...
      "service",
      "Create a tunnel to this service. The default local port is an ephemeral "
      "port. A specific local port can be used using the syntax --service "
      "<service>:<port> e.g. --service ssh:4242 to establish a tunnel to the "
      "ssh service and listen for connections to it on the local TCP port 4242",
  ...
  try {
    ...
    if (result.count("version")) {
      std::cout << edge_tunnel_client_version() << " (SDK version "
                << nabto_client_version() << ")" << std::endl;
      return 0;
    }
    ...
    if (result.count("delete-bookmark")) {
      if (!result.count("bookmark")) {
        std::cerr
            << "The argument --bookmark is required when deleting a bookmark."
            << std::endl;
      } else {
    ...
    else if (result.count("services") || result.count("service") ||
             result.count("users") || result.count("roles") ||
             result.count("set-role") || result.count("delete-user") ||
             result.count("get-user") || result.count("get-me") ||
...

    {
      ...
      if (Configuration::HasNoBookmarks()) {
        std::cerr << "No devices have been paired, start by pairing the client "
                     "with a device."
                  << std::endl;
        return 1;
      }
      ...
      if (!Device) {
        std::cerr << "The bookmark " << SelectedBookmark << " does not exist"
                  << std::endl;
        return 1;
      }
      ...
      std::cout << "Connected to the device " << Device->getFriendlyName()
                << std::endl;
