 ...
 *
 * Connections: The connection is the connection from this client to a device.
 * The connection is end to end encrypted. The connection can use several
 * channels to establish the connection to the device. There are three classes
 * of channels.
 *
...
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_BAD_RESPONSE;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_BAD_REQUEST;
...
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_FORBIDDEN;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_FUTURE_NOT_RESOLVED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_INVALID_ARGUMENT;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_INVALID_STATE;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_NOT_CONNECTED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_NOT_FOUND;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_NOT_IMPLEMENTED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_NO_CHANNELS;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError NABTO_CLIENT_EC_NO_DATA;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_OPERATION_IN_PROGRESS;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError NABTO_CLIENT_EC_PARSE;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_PORT_IN_USE;
...
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_NOT_ATTACHED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_TOKEN_REJECTED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_COULD_BLOCK;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_UNAUTHORIZED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_TOO_MANY_REQUESTS;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_UNKNOWN_PRODUCT_ID;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_UNKNOWN_DEVICE_ID;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_UNKNOWN_SERVER_KEY;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_CONNECTION_REFUSED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_DEVICE_INTERNAL_ERROR;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_PRIVILEGED_PORT;
 ...
 */
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_INTERNAL_ERROR;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientError
    NABTO_CLIENT_EC_DEVICE_INTERNAL_ERROR;
...
typedef enum NabtoClientConnectionType_ {
  ...
  NABTO_CLIENT_CONNECTION_TYPE_DIRECT // The connection is a direct connection.
                                      // The underlying channel is either p2p,
                                      // local or a direct candidate.
...
/**
 * A nabto stream is a bidirectional stream of bytes on top of a nabto
 * connection
 */
 ...
 */
typedef void (*NabtoClientFutureCallback)(NabtoClientFuture *future,
                                          NabtoClientError error, void *data);
 ...
 */
typedef void (*NabtoClientLogCallback)(const NabtoClientLogMessage *message,
                                       void *data);
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClient *NABTO_CLIENT_API nabto_client_new();
 ...
 *
 * This function is blocking until no more callbacks are in progress on the
 * event or callback queues.
 ...
 * @retval NABTO_CLIENT_EC_OK if the private key is created and available in
 * privateKey.
 * @retval NABTO_CLIENT_EC_UNKNOWN if the key could not be created for some
 * unknown reason. This should never happen. If the future has some more
 * specific error cases which can be acted upon programmatically then they will
 * be added as error codes.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 * - AppName: string
 * - AppVersion: string
 * - IgnoreDeviceNotAttached: (true|false)
 * ```
 *
 * If IgnoreDeviceNotAttached is set to true, the
 * nabto_client_connection_connect() will connect to the Basestation even if the
 * device is not attached. It will then observe the device attach state and wait
 * for the device to attach. When the device attaches, the client will connect
 * to it. Only then will the nabto_client_connection_connect() future resolve.
 *
 ...
 * ```
 *
 * Set the timeout for getting the first DTLS packet back from the device. This
 * is used to make a connection attempt fail faster if a route to the device is
 * believed to be open, when in fact it is not. Eg. if the device lost its
 * connection to the Nabto Server, but the server has yet to detect the
 * disconnect.
 * ```
 * - DtlsHelloTimeout: unsigned integer in milliseconds, default 10000
 * ```
 *
 * Set the timeout for a connect attempt. This is a upper limit on the time it
 * takes for nabto_client_connection_connect to resolve.
 * ```
 * - ConnectTimeout: unsigned integer in milliseconds, default 120000
 * ```
 *
 ...
 * @retval NABTO_CLIENT_EC_INVALID_ARGUMENT if the json is not understood. See
 * error log for more details.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_set_options(NabtoClientConnection *connection,
                                    const char *json);
 ...
 *
 * @param connection [in]  The connection.
 * @param json [out]  The json string representation of the current connection
 * options. The string should be freed with nabto_client_string_free().
 * @return NABTO_CLIENT_EC_OK if the options are present in the output string.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_get_options(NabtoClientConnection *connection,
                                    char **json);
 ...
 *
 * @param connection [in]  The connection.
 * @param productId [in]   The product id aka the id for the specific group of
 * devices.
 * @retval NABTO_CLIENT_EC_OK if the id was set.
 * @retval NABTO_CLIENT_EC_INVALID_STATE if the connection is not in the setup
 * phase
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_set_product_id(NabtoClientConnection *connection,
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_set_device_id(NabtoClientConnection *connection,
                                      const char *deviceId);
...
nabto_client_connection_set_server_connect_token(
    NabtoClientConnection *connection, const char *sct);
 ...
 *
 * @param connection [in] the connection
 * @param appName [in]  the application name. The string is copied into the
 * connection.
 * @retval NABTO_CLIENT_EC_OK on success
 * @retval NABTO_CLIENT_EC_INVALID_STATE if the connection is not in the setup
 * phase
 ...
 *
 * @param connection [in]  The connection
 * @param appVersion [in]  The application version, the string is copied into
...
nabto_client_connection_set_application_version(
    NabtoClientConnection *connection, const char *appVersion);
 ...
 *
 * @param connection [in] the connection
 * @param endpoint [in] the endpoint to use. The endpoint is a full https URL
 * e.g. `https://example.com:4242/foo.php`. The endpoint is copied into the
 * connection object.
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_set_private_key(NabtoClientConnection *connection,
 ...
 *
 * @param connection [in]  The connection.
 * @param fingerprint [out]  The fingerprint encoded as hex, the fingerprint has
 * to be freed using nabto_client_string_free.
 * @retval NABTO_CLIENT_EC_OK if the fingerprint was copied to the fingerprint
 * parameter.
 * @retval NABTO_CLIENT_EC_NOT_CONNECTED if the connection is not established
 * yet.
...
nabto_client_connection_get_device_fingerprint(
    NabtoClientConnection *connection, char **fingerprintHex);

/**
 * @deprecated use nabto_client_connection_get_device_fingerprint()
 *
 * Get a truncated fingerprint a truncated version of
 * nabto_client_connection_get_device_fingerprint()
 *
 * @param connection [in]  The connection.
 * @param fingerprintHex [out]  The fingerprint encoded as hex, the fingerprint
 * has to be freed using nabto_client_string_free.
 * @retval NABTO_CLIENT_EC_OK if the fingerprint was copied to the fingerprint
 * parameter.
 * @retval NABTO_CLIENT_EC_NOT_CONNECTED if the connection is not established
 * yet.
 ...
 *
 * @param connection [in]  The connection.
 * @param fingerprint [out]  The fingerprint, the fingerprint has to be freed
 * using nabto_client_string_free after use.
 * @retval NABTO_CLIENT_EC_OK if the fingerprint was copied to the fingerprint
 * parameter.
 ...
 *
 * @param connection [in]  The connection.
 * @param fingerprintHex [out]  The fingerprint, the fingerprint has to be freed
 * using nabto_client_string_free after use.
 * @retval NABTO_CLIENT_EC_OK if the fingerprint was copied to the fingerprint
 * parameter.
 * @retval NABTO_CLIENT_EC_INVALID_STATE if no client private key is set.
 ...
 * @deprecated use nabto_client_connection_get_client_fingerprint
 *
 * Get the fingerprint of the clients public key, same as
 * nabto_client_connection_get_client_fingerprint
 *
 * @param connection [in]  The connection.
 * @param fingerprintHex [out]  The fingerprint, the fingerprint has to be freed
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_get_type(NabtoClientConnection *connection,
 ...
 * @retval NABTO_CLIENT_EC_INVALID_ARGUMENT if the arguments are obviously
 * invalid. e.g. using port number 0
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_add_direct_candidate(NabtoClientConnection *connection,
                                             const char *hostname,
                                             uint16_t port);

/**
 * Inform the connection that no more direct endpoints will be added to the
 * connection.
 * @param connection [in]  The connection.
 ...
 *  - NABTO_CLIENT_EC_TIMEOUT if the connection timed out before any channels
 * could be created.
 *  - NABTO_CLIENT_EC_DEVICE_INTERNAL_ERROR if the device encountered an
 * internal error during the dtls connect attempt. This is most likely due to no
 * more connection resources available in the device.
 *  - NABTO_CLIENT_EC_STOPPED if the connection or the client is stopped.
 *
 * @param connection [in]  The connection.
 * @param future [in]  The future.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_connection_connect(
    NabtoClientConnection *connection, NabtoClientFuture *future);
 ...
 *  - NABTO_CLIENT_EC_STOPPED if the connection is closed or stopped or a parent
 * object is stopped. This way nabto_client_future_set_callback2 returns the
 * same value as nabto_client_future_wait when using the future.
 ...
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_connection_close(
    NabtoClientConnection *connection, NabtoClientFuture *future);

/**
 * Get error code for the local channel
 *
 * @param connection [in]  The connection on which the local channel is opened
 * for which the error should be retrieved.
 * @retval NABTO_CLIENT_EC_OK if the device was found using mDNS.
 * @retval NABTO_CLIENT_EC_NONE if mDNS discovery was not enabled for the
 * connection.
 * @retval NABTO_CLIENT_EC_NOT_FOUND if mDNS was enabled but the device was not
 * found.
 * @retval NABTO_CLIENT_EC_OPERATION_IN_PROGRESS if scanning is still in
 * progress
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 *
 * @param connection [in]  The connection on which the remote channel is opened
 * for which the error should be retrieved.
 * @retval NABTO_CLIENT_EC_OK  if a remote relay channel was made.
 * @retval NABTO_CLIENT_EC_NONE  if remote relay was not enabled.
 * @retval NABTO_CLIENT_EC_NOT_ATTACHED  if the device is not attached to the
 * basestation
 * @retval NABTO_CLIENT_EC_TIMEOUT  if a timeout occured when connecting to the
 * basestation.
 * @retval NABTO_CLIENT_EC_OPERATION_IN_PROGRESS  if the opening of the channel
 * is still in progress
 * @retval NABTO_CLIENT_EC_FORBIDDEN  if the basestation request is rejected.
 * @retval NABTO_CLIENT_EC_INVALID_STATE if required options is missing for the
 * remote connection.
 * @retval NABTO_CLIENT_EC_TOKEN_REJECTED  if the basestation rejects access to
 * a device based on either a valid formatted JWT token or a valid formatted SCT
 * token.
 * @retval NABTO_CLIENT_EC_DNS  if dns could not be resolved.
 * @retval NABTO_CLIENT_EC_UNKNOWN_SERVER_KEY if the server key is not known by
 * the basestation.
 * @retval NABTO_CLIENT_EC_UNKNOWN_PRODUCT_ID  if the product id is not known by
 * the basestation.
 * @retval NABTO_CLIENT_EC_UNKNOWN_DEVICE_ID  if the device id is not known by
 * the basestation.
 * @retval NABTO_CLIENT_EC_CONNECTION_REFUSED  if the client could not connect
 * to the basestation.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 *
 * @param connection [in]  The connection on which the direct channel is opened
 * for which the error should be retrieved.
 * @retval NABTO_CLIENT_EC_OK  if a direct candidate was found.
 * @retval NABTO_CLIENT_EC_NONE  direct candidates was not enabled.
 * @retval NABTO_CLIENT_EC_NOT_FOUND  If no direct candidates resulted in UDP
 * ping responses.
 * @retval NABTO_CLIENT_EC_OPERATION_IN_PROGRESS  if opening of the direct
 * candidate is in progress.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_get_info(NabtoClientConnection *connection,
                                 char **json);
 ...
 *  - NABTO_CLIENT_EC_NOT_FOUND if the password authentication feature is not
 * available on the device.
 *  - NABTO_CLIENT_EC_NOT_CONNECTED if the connection is not established yet.
 *  - NABTO_CLIENT_EC_OPERATION_IN_PROGRESS if a password authentication request
 * is already in progress on the connection.
 *  - NABTO_CLIENT_EC_TOO_MANY_REQUESTS if too many password attempts has been
 * made.
 ...
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API
nabto_client_connection_password_authenticate(NabtoClientConnection *connection,
                                              const char *username,
                                              const char *password,
...
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientConnectionEvent
    NABTO_CLIENT_CONNECTION_EVENT_CONNECTED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientConnectionEvent
    NABTO_CLIENT_CONNECTION_EVENT_CLOSED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientConnectionEvent
    NABTO_CLIENT_CONNECTION_EVENT_CHANNEL_CHANGED;
NABTO_CLIENT_DECL_PREFIX extern const NabtoClientConnectionEvent
    NABTO_CLIENT_CONNECTION_EVENT_WAITING_FOR_ATTACH;
 ...
 *
 * Future status:
 *  - NABTO_CLIENT_EC_OK       if the a connection event is available in the
 * event parameter.
 *  - NABTO_CLIENT_EC_STOPPED  if the listener is stopped.
 *
 * @param listener [in]  The listener.
 * @param future [in]    The future which is resolved when an connection event
 * is ready.
 * @param event [out]     The event which is overwritten when the future
 * resolves with NABTO_CLIENT_EC_OK.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API
nabto_client_listener_connection_event(NabtoClientListener *listener,
                                       NabtoClientFuture *future,
                                       NabtoClientConnectionEvent *event);
 ...
 * @retval NABTO_CLIENT_EC_OK if ok.
 * @retval NABTO_CLIENT_EC_INVALID_STATE if the listener is already initialized
 * or some other invalid state.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_connection_events_init_listener(NabtoClientConnection *connection,
                                             NabtoClientListener *listener);
 ...
 * Create a stream.
 *
 * @param connection [in]  The connection to make the stream on, the connection
 * needs to be kept alive until the stream has been freed.
 ...
 *  - NABTO_CLIENT_EC_STOPPED if the stream could not be created, e.g. the
 * handshake is stopped/aborted or the connection or client context is stopped.
 *  - NABTO_CLIENT_EC_NOT_CONNECTED if the connection is not established yet.
 *
 * @param stream [in]  The stream to connect.
 * @param future [in]  The future.
 * @param port [in]    The listening id/port to use for the stream. This is used
 * to distinguish streams in the other end, like a port number.
 *
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_stream_open(
    NabtoClientStream *stream, NabtoClientFuture *future, uint32_t port);

/**
 * Read exactly n bytes from a stream.
 *
 * If (readLength != bufferLength), the stream has reached a state where the
 * amount of bytes specified by bufferLength could NOT be read. The amount of
 * bytes which could be read is copied to the buffer and readLength is less than
 * bufferLength In any subsequent invocation, NABTO_CLIENT_EC_EOF will be
 * returned.
 *
 * Future status:
 ...
 * @param future [in]       The future that resolves when the read completes or
 * fails.
 * @param buffer [out]      The buffer to put data into. It needs to be kept
 * available until the future resolves.
 * @param bufferLength [in] The length of the output buffer.
 * @param readLength [out]  The actual number of bytes read. It needs to be kept
 * available until the future resolves.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_stream_read_all(
    NabtoClientStream *stream, NabtoClientFuture *future, void *buffer,
    size_t bufferLength, size_t *readLength);
 ...
 * @param future [in]        The future that resolves when the read completes or
 * fails.
 * @param buffer [out]       The buffer where bytes is copied to. It needs to be
 * kept available until the future resolves.
 * @param bufferLength [in]  The length of the output buffer.
 * @param readLength [out]   The actual number of read bytes. It needs to be
 * kept available until the future resolves.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_stream_read_some(
    NabtoClientStream *stream, NabtoClientFuture *future, void *buffer,
    size_t bufferLength, size_t *readLength);
 ...
 *   into the streaming buffers, but not neccessarily sent or acknowledgeg by
 * the other end yet.
 ...
 * @param future [in] The future that resolves when the write completes or
 * fails.
 * @param buffer [in] The input buffer with data to write to the stream, the
 * buffer needs to be kept alive until the future returns.
 * @param bufferLenth [in], length of the input data.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API
nabto_client_stream_write(NabtoClientStream *stream, NabtoClientFuture *future,
 ...
 *  - NABTO_CLIENT_EC_STOPPED if the stream is stopped.
 *  - NABTO_CLIENT_EC_OPERATION_IN_PROGRESS  if a stream close or stream write
 * is in progress.
 ...
 * @intro CoAP
 *
 * The CoAP implementation exhanges CoAP messages on top of a Nabto connection
 * between a client and a device.
 *
 * **CoAP example**:
 *
 * ```
 * NabtoClientCoap* coap = nabto_client_coap_new(connection, "GET",
 * "/temperature/living_room"); NabtoClientFuture* future =
 * nabto_client_coap_execute(coap); nabto_client_future_wait(future);
 ...
 * nabto_client_coap_get_response_payload(coap, &responsePayload,
 ...
 * @param connection [in]  The connection to make the CoAP request on, the
 * connection needs to be kept alive until the request has been freed.
 * @param method [in]      The CoAP method designator string. One of: GET, POST,
 * PUT, DELETE.
 * @param path [in]        The URI path element of the resource being requested.
 * It has to start with a '/' character. The string "/" is the root path.
 * @returns The created CoAP context, NULL if it could not be created (including
 * if method is invalid).
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientCoap *NABTO_CLIENT_API
nabto_client_coap_new(NabtoClientConnection *connection, const char *method,
                      const char *path);
 ...
 *
 * @param coap [in] The CoAP request to set request payload and content format
 * on.
 * @param contentFormat [in] See https://www.iana.org/assignments/core-parameters/core-parameters.xhtml, some
 * often used values are defined in NabtoClientCoapContentFormat.
 * @param payload [in] Data for the request encoded as specified in the
 * `contentFormat` parameter.
 * @param payloadLength [in] Length of the payload in bytes.
 * @return Returns NABTO_CLIENT_OK if the payload and content format were
 * successfully set.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_coap_set_request_payload(NabtoClientCoap *coap,
                                      uint16_t contentFormat,
                                      const void *payload,
 ...
 *  - NABTO_CLIENT_EC_TIMEOUT if the request timed out (took more than 2
 * minutes.)
 *  - NABTO_CLIENT_EC_STOPPED if the coap request or a parent object is stopped.
 *  - NABTO_CLIENT_EC_NOT_CONNECTED if the connection is not established yet.
 *
 * @param request [in] The CoAP request to execute.
 * @param future [in] The future completes when the CoAP request completes or
 * fails.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_coap_get_response_status_code(NabtoClientCoap *coap,
                                           uint16_t *statusCode);
 ...
 * @retval NABTO_DEVICE_EC_NO_DATA if the response does not have a content
 * format
 * @retval NABTO_DEVICE_EC_INVALID_STATE if no response is ready
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_coap_get_response_content_format(NabtoClientCoap *coap,
 ...
 * @retval NABTO_CLIENT_EC_OK if a payload exists and payload and payloadLength
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_coap_get_response_payload(NabtoClientCoap *coap, void **payload,
                                       size_t *payloadLength);
 ...
 *
 * TCP Tunnels allows tunnelling of tcp connections from a client to a device
 * over a nabto connection. Nabto Streams is used to stream the data reliably
 * over the Nabto Connection.
 *
 * The client opens a TCP listener which listens for incoming TCP connections on
 * the local port. When a connection is accepted by the TCP listener, a new
 * stream is created to the device. When the stream is created on the device,
 * the device opens a tcp connection to the specified service. Once this
 * connection is opened, TCP data flows from the TCP Client on the client side
 * to the TCP Server on the device side.
 */
 ...
 * Create a tunnel
 *
 * @param connection [in]  The connection to make the tunnel on, the connection
 * needs to be kept alive until the tunnel has been closed.
 ...
 *  - NABTO_CLIENT_EC_FORBIDDEN if target device did not allow opening a tunnel
 * to specified service for the current client
 *  - NABTO_CLIENT_EC_STOPPED if the tunnel is stopped.
 *  - NABTO_CLIENT_EC_NOT_CONNECTED if the connection is not established yet.
 *  - NABTO_CLIENT_EC_PRIVILEGED_PORT if the connection is not established
 * because the port is privileged and the user does not have access to start a
 * listening socket on that port number.
 *
 ...
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_tcp_tunnel_open(
    NabtoClientTcpTunnel *tunnel, NabtoClientFuture *future,
    const char *service, uint16_t localPort);
 ...
 * @param future [in] the future which resolves with the status of the
 * operation.
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_tcp_tunnel_close(
    NabtoClientTcpTunnel *tunnel, NabtoClientFuture *future);
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_tcp_tunnel_get_local_port(NabtoClientTcpTunnel *tunnel,
                                       uint16_t *localPort);
 ...
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_mdns_resolver_init_listener(NabtoClient *client,
                                         NabtoClientListener *listener,
                                         const char *subtype);
 ...
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API
nabto_client_listener_new_mdns_result(NabtoClientListener *listener,
                                      NabtoClientFuture *future,
                                      NabtoClientMdnsResult **mdnsResult);
 ...
 * asynchronous API-functions; a future resolves once such function has
 * completed. For more details about this topic, see the [Futures
 * Guide](/docs/developer/platforms/embedded/nabto_futures.html).
 *
 * Futures are introduced to unify the way return values and completion of
 * asynchronous functions are handled and to minimize the number of specialized
 * functions required in the APIs: Instead of having an asynchronous and
 * synchronous version of all functions, the API instead provides a single
 * version returning a future: For asynchronous behavior, a callback can then be
 * configured on the future - for synchronous behavior, the future provides a
 * `wait` function.
 *
 * In addition to futures, asynchronous functions that are expected to be
 * invoked recurringly introduces the concept of `listeners`, also elaborated in
 * the [Futures Guide](/docs/developer/platforms/embedded/nabto_futures.html).
 */
 ...
 * Free a future.
 *
 * Free must never be called on an unresolved future. If necessary, first cancel
 * the pending async operation to resolve the future as soon as possible. Use
 * the operation specific close/abort/free function as necessary such as
 * nabto_client_connection_close. Or use nabto_client_stop to cancel all pending
 * operations.
 *
 ...
 *
 * This must never be called after nabto_client_stop() since that would
 * mean the future cannot be resolved.
 *
 ...
 */
NABTO_CLIENT_DECL_PREFIX void NABTO_CLIENT_API nabto_client_future_set_callback(
    NabtoClientFuture *future, NabtoClientFutureCallback callback, void *data);

/**
 * Set a callback to be called when the future resolves.
 *
 * This replaces nabto_client_future_set_callback(). This new function
 * returns an error code which fixes the case where the client context
 * has been stopped meaning the future queue is closed and resolving
 * the future is impossible. In this case, this function will return
 * the stopped error instead of resolving the future.
 *
 * It is not allowed to call the following functions from the callback
 * as they could either crash the application or lead to the callback
 * thread being blocked.
 ...
 * @retval NABTO_CLIENT_EC_* the error code of the async operation if the future
 * is resolved
 * @retval NABTO_CLIENT_EC_COULD_BLOCK if called from a future callback.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_future_timed_wait(NabtoClientFuture *future,
                               nabto_client_duration_t duration);

/**
 * Retrieve error code from a future.
 * @param future [in] the future.
 * @retval NABTO_CLIENT_EC_FUTURE_NOT_RESOLVED if the future is not resolved
 * yet.
 * @retval NABTO_CLIENT_EC_* the error code of the async operation if the future
 * is resolved
 */
 ...
 * Nabto Edge uses `Futures` to manage return values and completion of
 * asynchronous API-functions; a future resolves once such function has
 * completed. Additionally, the Listener API supports asynchronous functions
 * that are expected to be invoked recurringly (see the [Futures
 * Guide](/docs/developer/api-reference/embedded-device-sdk/futures/Introduction.html)
 * for details).
 *
 * Listeners are created and freed through this general API. Once created, a
 * listener is initialized for use with a specific purpose, e.g. to listen for
 * [connection
 * events](/docs/developer/api-reference/plain-c-client-sdk/listeners/nabto_client_listener_connection_event.html),
 */
 ...
 * @intro Misc
 *
 * Functions for handling buffers, accessing error details, configuring logging
 * and getting the SDK version.
 */
 ...
 */
NABTO_CLIENT_DECL_PREFIX const char *NABTO_CLIENT_API nabto_client_version();

/**
 * Set a log callback.
 *
 * The log callback is called synchronously from the core, this means it is not
 * allowed to call any nabto_client_* functions from the log callback as that
 * would result in a deadlock. If it is needed to react on a log message, a
 * queue is needed such that the invocation of the nabto client SDK can occur
 * from another thread.
 *
 * To ensure safe operation, always call nabto_client_set_log_callback(client,
 * NULL, NULL) to unregister the log callback before it becomes invalid. This
 * prevents the SDK from attempting to call a function that is no longer valid.
 *
 * @param context [in]  The NabtoClient context to set the log callback on.
 * @param callback [in] The callback function or NULL to remove the callback.
 * @param data [in]     Passed to the callback along with the log data.
 * @return NABTO_CLIENT_EC_OK if ok.
 */
NABTO_CLIENT_DECL_PREFIX NabtoClientError NABTO_CLIENT_API
nabto_client_set_log_callback(NabtoClient *context,
                              NabtoClientLogCallback callback, void *data);
